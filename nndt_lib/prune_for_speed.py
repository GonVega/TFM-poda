...
def prune_tree_for_speed(tree: Tree, X_val, y_val, task='classification', metric='accuracy', tolerance=0.001):
    """
    Poda el árbol para reducir su tamaño y tiempo de inferencia, manteniendo el accuracy en validación lo más parecido posible.
    Args:
    ...
    def post_prune(node):
        ...
        # Evaluar accuracy tras la poda
        acc_after = tree.evaluate_model(X_val, y_val, task=task, metrics=[metric])[metric]
        # Restaurar si accuracy baja más de la tolerancia
        node._is_leaf = original_is_leaf
        node._child_nodes = original_children
        if hasattr(node, '_leaf_value'):
            del node._leaf_value
        acc_before = tree.evaluate_model(X_val, y_val, task=task, metrics=[metric])[metric]
        if acc_after >= acc_before - tolerance:
            # Poda aceptada
            node._is_leaf = True
            node._child_nodes = []
            node._leaf_value = node._leaf_value if hasattr(node, '_leaf_value') else 0
